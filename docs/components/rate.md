...
<template>
  <div style="display: flex; flex-direction: column; gap: 16px;">
    ...
    <div style="display: flex; gap: 8px;">
      <ad-button @click="value = 0">Reset</ad-button>
      <ad-button @click="value = 5">Max</ad-button>
    </div>
  </div>
</template>
...
| 事件名 | 说明 | 回调参数 |
| --- | --- | --- |
| update:value | 值改变时触发（v-model） | `(value: number) => void` |
...
5. 在受控模式下，需要同时提供 `value` 和 `@update:value` 事件（或使用 `v-model:value`）
