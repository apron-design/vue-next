...
| 事件名 | 说明 | 参数 |
| --- | --- | --- |
| update:value | 值改变时触发 | `(value: string)` |
| change | 值改变时触发 | `(value: string)` |
| finish | 输入完成时触发（满足长度时自动触发） | `(value: string)` |
| complete | 输入完成时触发（同 finish） | `(value: string)` |
| status-reset | error 状态下按退格键重置时触发 | - |

### InputOtp Methods

通过 ref 可以获取到 InputOtp 实例并调用实例方法。
